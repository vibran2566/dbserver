(() => {
  // --- dev hot-reload: stop an earlier run if it exists ---
  if (window.__USERNAME_TRACKER__?.stop) {
  ...
  // Version label (from cache or GM_info)
  const UI_VER = (() => {
    try {
  ...
  // --- helpers: server key + normalize ---
  function resolveServerKey() {
    const sid = new URLSearchParams(location.search).get('serverId') || '';
    const m = sid.match(/^(us|eu)-(1|5|20)/i);
    const region = m ? m[1].toLowerCase() : 'us';
    const amount = m ? m[2] : '1';
    return { region, amount, serverKey: region + '-' + amount };
  }
  ...
  function normalizeFilterSort(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      let sRaw = (p.size ?? p.snakeSize ?? p.length ?? 0);
      let mvRaw = (p.monetaryValue ?? p.value ?? p.money ?? p.cash ?? 0);

      let sizeNum = Number(sRaw); if (!isFinite(sizeNum)) sizeNum = 0;
      let mvNum   = Number(mvRaw); if (!isFinite(mvNum))   mvNum   = 0;

      if (mvNum > 0 && sizeNum > 2) {
        out.push({ name, privyId, size: sizeNum, monetaryValue: mvNum });
      }
    }
    out.sort((a,b) => (b.monetaryValue||0)-(a.monetaryValue||0) || (b.size||0)-(a.size||0) || String(a.name||'').localeCompare(String(b.name||'')));
    return out;
  }
...
`;
  ...
  function dbShowAlertToast(msg){
    ...
    const title = document.createElement('div'); title.className = 'db-alert-title'; title.textContent = 'ALERT:';
    const body  = document.createElement('div'); body.className  = 'db-alert-body';  body.textContent  = String(msg || '');
  ...
  function dbAlertUid(a){
    const cand = [a.id,a.uuid,a.uid,a.ts,a.timestamp,a.time,a.createdAt,a.expiresAt,a.message].filter(Boolean)[0];
  ...
  async function fetchLeaderboard() {
    ...
    const filtered = entries
      .filter(p => (p && p.name) && (p.monetaryValue||0) > 0 && (p.size||0) > 2)
  ...
  async function fetchFilteredSortedList() {
    const sk = resolveServerKey();
    try {
      const aggUrl = `${GAME_API_BASE}/api/game/leaderboard?serverKey=${encodeURIComponent(sk.serverKey)}`;
      const res = await fetch(aggUrl, { cache: 'no-store' });
      if (res.ok) {
        const j = await res.json();
        const arr = Array.isArray(j.entries) ? j.entries : [];
        const list = normalizeFilterSort(arr);
        if (list.length > 0) return list;
      }
    } catch {}
    try {
      const rawUrl = `https://damnbruh-game-server-instance-${sk.amount}-${sk.region}.onrender.com/players`;
      const r2 = await fetch(rawUrl, { cache: 'no-store' });
      if (r2.ok) {
        const body = await r2.json();
        const base = Array.isArray(body) ? body : (Array.isArray(body.players) ? body.players : []);
  ...
  // --- styles for the leaderboard + modal drag ---
...
#username-leaderboard{
  position:fixed;z-index:2147483646;left:16px;top:16px;width:220px;color:#fff;background:rgba(0,0,0,.75);
  border:1px solid rgba(255,255,255,.12);border-radius:12px;box-shadow:0 6px 18px rgba(0,0,0,.35);
  font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:13px;line-height:1.35;padding:10px 12px 8px;backdrop-filter:blur(6px)
}
#username-leaderboard-header{
  position:relative;padding-top:14px;text-align:left;font-weight:700;letter-spacing:.06em;margin-bottom:8px;padding-left:28px
}
#username-leaderboard-header::before{
  content:'';position:absolute;width:10px;height:10px;left:5px;top:17.5px;background:#22c55e;border-radius:50%;box-shadow:0 0 6px #22c55e
}
.ub-top-drag{
...
.ub-top3-popup{
  ...
  min-width:160px;background:rgba(0,0,0,.92);color:#fff;border:1px solid rgba(255,255,255,.16);border-radius:8px;
  box-shadow:0 10px 30px rgba(0,0,0,.7);padding:8px;font-size:11px;display:none;z-index:99999;white-space:normal
}
...
.db-top-drag{
  ...
  width:44px; height:8px; border-radius:6px; background:rgba(255,255,255,.12); cursor:move; z-index:2;
...
`;
  ...
  function ensureBox() {
    ...
    // Reuse existing DOM to guarantee single instance even if code is injected twice
  ...
  // expose minimal control / info for dev
  window.__USERNAME_TRACKER__ = { stop, endpoint: playersEndpoint, version: UI_VER };
